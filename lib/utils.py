...
def save_to_file(root_dic, tracker,ret):


    # if len(ret) > 0:
    #     # print(np.shape(ret))
    #     for d in np.concatenate(ret):
    #         d = d.astype(np.int32)
    #         file_name = d[4]
    # print(tracker.id)
    ...
    if len(filter_face_addtional_attribute_list) > 0:
        ...
        # 通过带入的口罩参数检查是否戴口罩
        if filter_face_addtional_attribute_list[-1][-1] == 0:
            tmp_root = root_dic+'/mask'
            mkdir(tmp_root)
            cv2.imwrite("{0}/{1}.jpg".format(tmp_root, 'ID_'+str(tracker.id)), score_reverse_sorted_list[0][0])


        else:
            tmp_root = root_dic + '/no_mask'
            mkdir(tmp_root)
